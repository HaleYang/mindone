...
from mindspore.common.api import _pynativate_executor as ms_pyexecutor
...
from .infer.transformer_sd3_cache import SD3Transformer2DModelCache


class StableDiffusion3PipelineBoost(StableDiffusion3Pipeline):
    def __init__(
        self, 
        transformer: SD3Transformer2DModelCache, 
        ...
        self.transformer: SD3Transformer2DModelCache

        self.tgate = 20
        self.use_cache_and_tgate = False
        self.use_todo = False

    def enable_boost(self, use_cache_and_tgate: bool = True, use_todo: bool = False):
        for transformer_block in self.transformer.transformer_blocks:
            transformer_block.attn.use_todo = use_todo
    ...
    def __call__(
        ...
        if self.use_cache_and_tgate:
            ...
            delta_cache_hidden = ops.zeros([2, max_sequence_length + 77, 1536], dtype=ms.flaot16)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if not self.use_cache_and_tgate:
                    ...
                    ms_pyexecutor.sync()
                    ms_pyexecutor.set_sync_for_graph(False)
                    ...
                    ms_pyexecutor.set_sync_for_graph(True)
                else:
                    if i < self.tgate:
                        latent_model_input = ops.cat([latents] * 2) if self.do_classifier_free_guidance else latents
                    else:
